...
daterange = f"{year:04}-01-01T00:00:00.000/{year:04}-{month:02}-{day:02}T23:59:59.999"
...
df = df[df["creator"].isin(creators.keys())]

df["creator"] = df["creator"].map(creators)
...
working_hours = 8 * cal.get_working_days_delta(date(year, 1, 1), date(year, month, day))
